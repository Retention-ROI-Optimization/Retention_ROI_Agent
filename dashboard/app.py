...
# ============================================================
# [PATCH] Composite table-cell i18n for mixed action strings
# Examples generated by the recommendation pipeline can look like
# "Generic retention offer · 중강도 · Monitor (>60d)". These are not
# widget labels; they are table cell values composed from multiple tokens.
# Translate them at display time without mutating source artifacts.
# ============================================================
_COMPOSITE_CELL_VALUE_LABELS_PATCH: dict[str, dict[str, str]] = {
    "ko": {
        "Generic retention offer": "기본 리텐션 혜택",
        "generic retention offer": "기본 리텐션 혜택",
        "Basic retention offer": "기본 리텐션 혜택",
        "basic retention offer": "기본 리텐션 혜택",
        "Coupon offer": "쿠폰 혜택",
        "Discount offer": "할인 혜택",
        "Personalized coupon": "개인 맞춤 쿠폰",
        "Service recovery": "서비스 회복 안내",
        "Loyalty reward": "충성 고객 보상",
        "Monitor": "점검",
        "monitor": "점검",
        "Observe": "관찰",
        "High intensity": "고강도",
        "Medium intensity": "중강도",
        "Low intensity": "저강도",
        "high intensity": "고강도",
        "medium intensity": "중강도",
        "low intensity": "저강도",
    ...
    "en": {
        ...
        "저강도": "Low intensity",
        "높은 강도": "High intensity",
        "중간 강도": "Medium intensity",
        "낮은 강도": "Low intensity",
        "강한 개입": "High intensity",
        "보통 개입": "Medium intensity",
        "약한 개입": "Low intensity",
    ...
    "ja": {
        ...
        "generic retention offer": "基本リテンション特典",
        "Basic retention offer": "基本リテンション特典",
        "basic retention offer": "基本リテンション特典",
        "Coupon offer": "クーポン特典",
        "Discount offer": "割引特典",
        "Personalized coupon": "個別クーポン",
        "Service recovery": "サービス回復メッセージ",
        "Loyalty reward": "ロイヤルティ特典",
        "Monitor": "観察",
        "monitor": "観察",
        "Observe": "観察",
        "고강도": "高強度",
...
for _lang, _mapping in _COMPOSITE_CELL_VALUE_LABELS_PATCH.items():
    VALUE_LABELS.setdefault(_lang, {}).update(_mapping)
# ============================================================
# [/PATCH]
# ============================================================
...
def _translate_cell_value(value: Any) -> str:
    """Translate table cell values without changing the source artifacts.

    A lot of visible text in the dashboard is not a Streamlit widget label but
    a value generated by the training/recommendation pipeline. Some cells are
    composite strings such as "Generic retention offer · 중강도 · Monitor (>60d)".
    Exact dictionary lookup cannot translate those, so this function translates
    each visible fragment and then performs safe token replacement.
    """
    raw = str(value)
    stripped = raw.strip()
    if stripped == "":
    ...
    def _exact_translate(fragment: str) -> str | None:
        ...
        if not frag:
            return ""
        norm = _normalize_i18n_key(frag)
        for src, dst in value_labels.items():
            src_text = str(src)
            if frag == src_text or norm == _normalize_i18n_key(src_text):
                return str(dst)
        return None

    def _flexible_ascii_pattern(src: str) -> str:
        """Match generic_retention_offer, Generic retention offer, generic-retention-offer."""
        escaped_parts = [re.escape(part) for part in re.split(r"[\s_\-]+", src.strip()) if part]
        if not escaped_parts:
            return re.escape(src)
        return r"[\s_\-]+".join(escaped_parts)
    ...
    def _translate_fragment(fragment: str) -> str:
        ...
        exact = _exact_translate(core)
        if exact is not None:
            return leading + exact + trailing

        # Preserve SLA/window suffixes: Monitor (>60d) -> 観察 (>60d)
        suffix_match = re.match(r"^(.*?)(\s*\([^)]*\)\s*)$", core)
        if suffix_match:
            base, suffix = suffix_match.groups()
            base_exact = _exact_translate(base.strip())
            if base_exact is not None:
                return leading + base_exact + suffix + trailing

        out = core

        # Generated Korean explanation phrases.
        for src, dst in sorted(phrase_labels.items(), key=lambda item: len(str(item[0])), reverse=True):
            src_text = str(src)
            if src_text:
                out = out.replace(src_text, str(dst))

        # Known value tokens embedded inside a longer cell.
        for src, dst in sorted(value_labels.items(), key=lambda item: len(str(item[0])), reverse=True):
            src_text = str(src)
            if not src_text or len(src_text) <= 1:
                continue
            dst_text = str(dst)

            # Korean/Japanese tokens do not need word boundaries.
            if re.search(r"[가-힣ぁ-んァ-ン一-龥]", src_text):
                out = out.replace(src_text, dst_text)
                continue

            # ASCII tokens: support snake_case/space/hyphen variants and ignore case.
            pattern = _flexible_ascii_pattern(src_text)
            out = re.sub(
    ...
    # Translate composite action strings fragment by fragment. Keep separators stable.
    # This specifically catches cells like "Generic retention offer · 중강도 · Monitor (>60d)".
    parts = re.split(r"(\s+·\s+|\s+\|\s+|\s+→\s+|\s+>\s+)", stripped)
    if len(parts) > 1:
